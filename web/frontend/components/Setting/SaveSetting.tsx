...
interface SaveSettingProps {
  fetchQuoteEntity: () => void;
}

const SaveSetting = ({fetchQuoteEntity,}: SaveSettingProps) => {
    ...
    return (
      ...
      {state.currentSetting && <>
        <ButtonGroup>
          <Button destructive onClick={unchamgeSetting} >UnChange</Button>
          <Button primary onClick={updateSetting} >Save All Setting</Button>
        </ButtonGroup>
      </>}
      </>
    )
