...
function Index() {
  ...
  const [variantID, setVariantID] = useState(variant_selected_id)
  // const [clicked, setClicked] = useState(false)
  // const queryParameters = new URLSearchParams(window.location.search)
  //   const variant_ID = queryParameters.get('variant')
  //   console.log("Variant_ID",variant_ID);
  //   setVariantID(queryParameters.get('variant'))
  // document.getElementsByClassName('product-form__input')[0].addEventListener('click',function(event:unknown) {
...
// },[clicked])
  ...
  useEffect(() => {
    ...
    void fetch(`/apps/vify_rfq-f/quote_setting?variant_selected_id=${variant_selected.id}`)
      ...
      {setting.show &&
          <div>
              <Button style={{backgroundColor: "#212121"}} variant="contained" sx={{width: '100%'}}
                      onClick={() => handleChangeModal('request')}>Request For Quote 1.6</Button>
            {modal === 'request' &&
